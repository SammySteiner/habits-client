...
export default (props) => {
  const actionsInputs = props.actions.map( (action, i) => {
    return (
      <Container text key={i}>
        <Form.Field >
          <Form.Input label={`Action ${i + 1}:`} placeholder={`What specifically will you do to complete action ${i + 1}?`} value={props.goals[props.goalIndex].actions[i].description} onChange={e => props.handleChange( ['actDesc', props.goalIndex, i], e )}/>
